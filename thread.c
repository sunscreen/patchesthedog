...
void handle_signal(int signal) {
    ...
    switch (signal) {
    case 1:
        printf("HANGUP Recieved signal %d\n",signal);
        config_reread_config(&mutex);
        break;
    case 2:
        printf("CTRL-C Recieved signal %d\n",signal);
        exit(0);
        break;
    }
...
static void _catch_signals(void)
{
    ...
    printf("My handler tid is: %li parent pid %d\n",ptid, getpid());
    ...
    sigaction(SIGKILL,&ignore_sa,NULL);
    sigaction(SIGSTOP,&ignore_sa,NULL);
    sigaction(SIGSEGV,&ignore_sa,NULL);
    sigaction(SIGCHLD,&ignore_sa,NULL);
    sigaction(SIGBUS,&ignore_sa,NULL);
...
static void *_start_routine(void *arg)
{
    ...
    //_block_signals();
    if (strncmp(thread->name,"Events Thread",12) == 0) {
    _catch_signals();
    }
